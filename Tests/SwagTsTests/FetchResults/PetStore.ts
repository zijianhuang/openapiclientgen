export namespace MyNS {
	...
	export class PetClient {
		...
		UpdatePet(requestBody: Pet): Promise<Pet> {
			return fetch(this.baseUri + 'pet', { method: 'put', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		AddPet(requestBody: Pet): Promise<Pet> {
			return fetch(this.baseUri + 'pet', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		FindPetsByStatus(status: PetStatus | null | undefined): Promise<Array<Pet>> {
			return fetch(this.baseUri + 'pet/findByStatus?status=' + status, { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		FindPetsByTags(tags: Array<string> | null | undefined): Promise<Array<Pet>> {
			return fetch(this.baseUri + 'pet/findByTags?' + tags?.map(z => `tags=${encodeURIComponent(z)}`).join('&'), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		GetPetById(petId: string): Promise<Pet> {
			return fetch(this.baseUri + 'pet/' + petId, { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	...
	export class StoreClient {
		...
		GetInventory(): Promise<{[id: string]: number }> {
			return fetch(this.baseUri + 'store/inventory', { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		PlaceOrder(requestBody: Order): Promise<Order> {
			return fetch(this.baseUri + 'store/order', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		GetOrderById(orderId: string): Promise<Order> {
			return fetch(this.baseUri + 'store/order/' + orderId, { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	...
	export class UserClient {
		...
		CreateUsersWithListInput(requestBody: Array<User>): Promise<User> {
			return fetch(this.baseUri + 'user/createWithList', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		GetUserByName(username: string): Promise<User> {
			return fetch(this.baseUri + 'user/' + (username == null ? '' : encodeURIComponent(username)), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
