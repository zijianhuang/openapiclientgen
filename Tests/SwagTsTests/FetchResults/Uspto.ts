export namespace MyNS {
	...
	export class MetadataClient {
		...
		ListDataSets(): Promise<DataSetList> {
			return fetch(this.baseUri + '', { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		ListSearchableFields(dataset: string, version: string): Promise<string> {
			return fetch(this.baseUri + (dataset == null ? '' : encodeURIComponent(dataset)) + '/' + (version == null ? '' : encodeURIComponent(version)) + '/fields', { method: 'get' }).then(d => {if (d.status<=204) return d.status == 204 ? null : d.text(); throw d;});
		}
	}
