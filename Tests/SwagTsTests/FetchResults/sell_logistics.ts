export namespace MyNS {
	...
	export class ShipmentClient {
		 ...
		 * @return {void} 
		 */
		CreateFromShippingQuote(requestBody: CreateShipmentFromQuoteRequest): Promise<Response> {
			return fetch(this.baseUri + 'shipment/create_from_shipping_quote', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) });
		}
		...
		GetShipment(shipmentId: string): Promise<Shipment> {
			return fetch(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		 ...
		 * @return {void} Success
		 */
		DownloadLabelFile(shipmentId: string): Promise<Response> {
			return fetch(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/download_label_file', { method: 'get' });
		}
		...
		CancelShipment(shipmentId: string): Promise<Shipment> {
			return fetch(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/cancel', { method: 'post' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	}
	...
	export class Shipping_quoteClient {
		 ...
		 * @return {void} 
		 */
		CreateShippingQuote(requestBody: ShippingQuoteRequest): Promise<Response> {
			return fetch(this.baseUri + 'shipping_quote', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) });
		}
		...
		GetShippingQuote(shippingQuoteId: string): Promise<ShippingQuote> {
			return fetch(this.baseUri + 'shipping_quote/' + (shippingQuoteId == null ? '' : encodeURIComponent(shippingQuoteId)), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	}
