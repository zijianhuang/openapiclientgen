export namespace MyNS {
	...
	export class Listing_violation_summaryClient {
		 ...
		 * @return {void} Success
		 */
		GetListingViolationsSummary(compliance_type: string | null | undefined): Promise<Response> {
			return fetch(this.baseUri + 'listing_violation_summary?compliance_type=' + (compliance_type == null ? '' : encodeURIComponent(compliance_type)), { method: 'get' });
		}
	}
	...
	export class Listing_violationClient {
		 ...
		 * @return {void} Success
		 */
		GetListingViolations(compliance_type: string | null | undefined, offset: string | null | undefined, listing_id: string | null | undefined, limit: string | null | undefined, filter: string | null | undefined): Promise<Response> {
			return fetch(this.baseUri + 'listing_violation?compliance_type=' + (compliance_type == null ? '' : encodeURIComponent(compliance_type)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)) + '&listing_id=' + (listing_id == null ? '' : encodeURIComponent(listing_id)) + '&limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&filter=' + (filter == null ? '' : encodeURIComponent(filter)), { method: 'get' });
		}
		 ...
		 * @return {void} 
		 */
		SuppressViolation(requestBody: SuppressViolationRequest): Promise<Response> {
			return fetch(this.baseUri + 'suppress_listing_violation', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) });
		}
	}
