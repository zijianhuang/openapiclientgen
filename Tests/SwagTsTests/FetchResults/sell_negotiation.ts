export namespace MyNS {
	...
	export class OfferClient {
		 ...
		 */
		FindEligibleItems(limit: string | null | undefined, offset: string | null | undefined): Promise<PagedEligibleItemCollection> {
			return fetch(this.baseUri + 'find_eligible_items?limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		 ...
		 */
		SendOfferToInterestedBuyers(requestBody: CreateOffersRequest): Promise<SendOfferToInterestedBuyersCollectionResponse> {
			return fetch(this.baseUri + 'send_offer_to_interested_buyers', { method: 'post', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	}
