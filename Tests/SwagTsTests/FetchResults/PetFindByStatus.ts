export namespace MyNS {
	...
	export class Misc {
		...
		FindPetsByStatus2(status: PetStatus): Promise<Array<Pet>> {
			return fetch(this.baseUri + 'pet/findByStatus2?status=' + status, { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
		...
		FindPetsByStatus3(status: PetStatus): Promise<Array<Pet>> {
			return fetch(this.baseUri + 'pet/findByStatus3?status=' + status, { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	}
