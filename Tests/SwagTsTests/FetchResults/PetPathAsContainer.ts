export namespace MyNS {
	...
	export class PetFindByStatusClient {
		...
		GetByStatus(status: Array<PetStatus>): Promise<Array<Pet>> {
			return fetch(this.baseUri + 'pet/findByStatus?' + status.map(z => `status=${z}`).join('&'), { method: 'get' }).then(d => {if (d.status<=202) return d.json(); else if (d.status==204) return null; throw d;});
		}
	}
