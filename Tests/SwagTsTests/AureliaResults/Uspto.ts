...
export namespace MyNS {
	...
	export class MetadataClient {
		...
		ListDataSets(): Promise<DataSetList> {
			return this.http.get('', {}).then(d => {if (d.status<=204) return d.json(); throw d;});
		}
		...
		ListSearchableFields(dataset: string, version: string): Promise<string> {
			return this.http.get('' + (dataset == null ? '' : encodeURIComponent(dataset)) + '/' + (version == null ? '' : encodeURIComponent(version)) + '/fields', {}).then(d => {if (d.status<=204) return d.text(); throw d;});
		}
	}
