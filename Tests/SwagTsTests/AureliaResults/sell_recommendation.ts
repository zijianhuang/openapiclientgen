...
export namespace MyNS {
	...
	export class Listing_recommendationClient {
		...
		FindListingRecommendations(filter: string | null | undefined, limit: string | null | undefined, offset: string | null | undefined, requestBody: FindListingRecommendationRequest): Promise<Response> {
			return this.http.post('find?filter=' + (filter == null ? '' : encodeURIComponent(filter)) + '&limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
	}
