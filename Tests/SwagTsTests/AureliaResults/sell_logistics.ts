...
export namespace MyNS {
	...
	export class ShipmentClient {
		...
		CreateFromShippingQuote(requestBody: CreateShipmentFromQuoteRequest): Promise<Response> {
			return this.http.post('shipment/create_from_shipping_quote', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		GetShipment(shipmentId: string): Promise<Shipment> {
			return this.http.get('shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)), {}).then(d => {if (d.status<=204) return d.json(); throw d;});
		}
		...
		DownloadLabelFile(shipmentId: string): Promise<Response> {
			return this.http.get('shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/download_label_file', {});
		}
		...
		CancelShipment(shipmentId: string): Promise<Shipment> {
			return this.http.post('shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/cancel', null, {}).then(d => {if (d.status<=204) return d.json(); throw d;});
		}
	}
	...
	export class Shipping_quoteClient {
		...
		CreateShippingQuote(requestBody: ShippingQuoteRequest): Promise<Response> {
			return this.http.post('shipping_quote', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		GetShippingQuote(shippingQuoteId: string): Promise<ShippingQuote> {
			return this.http.get('shipping_quote/' + (shippingQuoteId == null ? '' : encodeURIComponent(shippingQuoteId)), {}).then(d => {if (d.status<=204) return d.json(); throw d;});
		}
	}
