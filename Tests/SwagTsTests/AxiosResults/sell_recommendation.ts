...
export namespace MyNS {
	...
	export class Listing_recommendationClient {
		 ...
		 * @return {void} Success
		 */
		FindListingRecommendations(filter: string | null | undefined, limit: string | null | undefined, offset: string | null | undefined, requestBody: FindListingRecommendationRequest): Promise<AxiosResponse<string>> {
			return Axios.post(this.baseUri + 'find?filter=' + (filter == null ? '' : encodeURIComponent(filter)) + '&limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, responseType: 'text' });
		}
	}
