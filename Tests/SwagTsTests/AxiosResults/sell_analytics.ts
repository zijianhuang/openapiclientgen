...
export namespace MyNS {
	...
	export class Customer_service_metricClient {
		...
		GetCustomerServiceMetric(customer_service_metric_type: string, evaluation_marketplace_id: string, evaluation_type: string): Promise<GetCustomerServiceMetricResponse> {
			return Axios.get<GetCustomerServiceMetricResponse>(this.baseUri + 'customer_service_metric/' + (customer_service_metric_type == null ? '' : encodeURIComponent(customer_service_metric_type)) + '/' + (evaluation_type == null ? '' : encodeURIComponent(evaluation_type)) + '&evaluation_marketplace_id=' + (evaluation_marketplace_id == null ? '' : encodeURIComponent(evaluation_marketplace_id)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
	...
	export class Seller_standards_profileClient {
		...
		FindSellerStandardsProfiles(): Promise<FindSellerStandardsProfilesResponse> {
			return Axios.get<FindSellerStandardsProfilesResponse>(this.baseUri + 'seller_standards_profile', {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
		...
		GetSellerStandardsProfile(cycle: string, program: string): Promise<StandardsProfile> {
			return Axios.get<StandardsProfile>(this.baseUri + 'seller_standards_profile/' + (program == null ? '' : encodeURIComponent(program)) + '/' + (cycle == null ? '' : encodeURIComponent(cycle)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
	...
	export class Traffic_reportClient {
		...
		GetTrafficReport(dimension: string | null | undefined, filter: string | null | undefined, metric: string | null | undefined, sort: string | null | undefined): Promise<Report> {
			return Axios.get<Report>(this.baseUri + 'traffic_report?dimension=' + (dimension == null ? '' : encodeURIComponent(dimension)) + '&filter=' + (filter == null ? '' : encodeURIComponent(filter)) + '&metric=' + (metric == null ? '' : encodeURIComponent(metric)) + '&sort=' + (sort == null ? '' : encodeURIComponent(sort)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
