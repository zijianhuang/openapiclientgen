...
export namespace MyNS {
	...
	export class OfferClient {
		...
		FindEligibleItems(limit: string | null | undefined, offset: string | null | undefined): Promise<PagedEligibleItemCollection> {
			return Axios.get<PagedEligibleItemCollection>(this.baseUri + 'find_eligible_items?limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
		...
		SendOfferToInterestedBuyers(requestBody: CreateOffersRequest): Promise<SendOfferToInterestedBuyersCollectionResponse> {
			return Axios.post<SendOfferToInterestedBuyersCollectionResponse>(this.baseUri + 'send_offer_to_interested_buyers', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
