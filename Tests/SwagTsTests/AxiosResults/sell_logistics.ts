...
export namespace MyNS {
	...
	export class ShipmentClient {
		 ...
		 * @return {void} 
		 */
		CreateFromShippingQuote(requestBody: CreateShipmentFromQuoteRequest): Promise<AxiosResponse<string>> {
			return Axios.post(this.baseUri + 'shipment/create_from_shipping_quote', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, responseType: 'text' });
		}
		...
		GetShipment(shipmentId: string): Promise<Shipment> {
			return Axios.get<Shipment>(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
		 ...
		 * @return {void} Success
		 */
		DownloadLabelFile(shipmentId: string): Promise<AxiosResponse<string>> {
			return Axios.get(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/download_label_file', { responseType: 'text' });
		}
		...
		CancelShipment(shipmentId: string): Promise<Shipment> {
			return Axios.post<Shipment>(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/cancel', null, {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
	...
	export class Shipping_quoteClient {
		 ...
		 * @return {void} 
		 */
		CreateShippingQuote(requestBody: ShippingQuoteRequest): Promise<AxiosResponse<string>> {
			return Axios.post(this.baseUri + 'shipping_quote', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, responseType: 'text' });
		}
		...
		GetShippingQuote(shippingQuoteId: string): Promise<ShippingQuote> {
			return Axios.get<ShippingQuote>(this.baseUri + 'shipping_quote/' + (shippingQuoteId == null ? '' : encodeURIComponent(shippingQuoteId)), {}).then(d => {if (d.status<=204) return d.data; throw d;});
		}
	}
