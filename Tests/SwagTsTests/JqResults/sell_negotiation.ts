...
export namespace MyNS {
	...
	export class OfferClient {
		 ...
		 */
		FindEligibleItems(limit: string | null | undefined, offset: string | null | undefined, callback: (data : PagedEligibleItemCollection) => any) {
			this.httpClient.get(this.baseUri + 'find_eligible_items?limit=' + (limit == null ? '' : encodeURIComponent(limit)) + '&offset=' + (offset == null ? '' : encodeURIComponent(offset)), callback, this.error, this.statusCode);
		}
		 ...
		 */
		SendOfferToInterestedBuyers(callback: (data : SendOfferToInterestedBuyersCollectionResponse) => any, requestBody: CreateOffersRequest) {
			this.httpClient.post(this.baseUri + 'send_offer_to_interested_buyers', requestBody, callback, this.error, this.statusCode, 'application/json;charset=UTF-8');
		}
	}
