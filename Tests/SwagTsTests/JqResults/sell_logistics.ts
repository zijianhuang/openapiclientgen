...
export namespace MyNS {
	...
	export class ShipmentClient {
		 ...
		 * @return {void} 
		 */
		CreateFromShippingQuote(callback: (data : void) => any, requestBody: CreateShipmentFromQuoteRequest) {
			this.httpClient.post(this.baseUri + 'shipment/create_from_shipping_quote', requestBody, callback, this.error, this.statusCode, 'application/json;charset=UTF-8');
		}
		 ...
		 */
		GetShipment(shipmentId: string, callback: (data : Shipment) => any) {
			this.httpClient.get(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)), callback, this.error, this.statusCode);
		}
		 ...
		 * @return {void} Success
		 */
		DownloadLabelFile(shipmentId: string, callback: (data : void) => any) {
			this.httpClient.get(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/download_label_file', callback, this.error, this.statusCode);
		}
		 ...
		 * @return {Shipment} Success
		 */
		CancelShipment(shipmentId: string, callback: (data : Shipment) => any) {
			this.httpClient.post(this.baseUri + 'shipment/' + (shipmentId == null ? '' : encodeURIComponent(shipmentId)) + '/cancel', null, callback, this.error, this.statusCode, 'application/json;charset=UTF-8');
		}
	}

	export class Shipping_quoteClient {
		constructor(private baseUri: string = HttpClient.locationOrigin, private httpClient: HttpClientBase = new HttpClient(), private error?: (xhr: JQueryXHR, ajaxOptions: string, thrown: string) => any, private statusCode?: { [key: string]: any; }) {
		}
		 ...
		 * @return {void} 
		 */
		CreateShippingQuote(callback: (data : void) => any, requestBody: ShippingQuoteRequest) {
			this.httpClient.post(this.baseUri + 'shipping_quote', requestBody, callback, this.error, this.statusCode, 'application/json;charset=UTF-8');
		}
		 ...
		 */
		GetShippingQuote(shippingQuoteId: string, callback: (data : ShippingQuote) => any) {
			this.httpClient.get(this.baseUri + 'shipping_quote/' + (shippingQuoteId == null ? '' : encodeURIComponent(shippingQuoteId)), callback, this.error, this.statusCode);
		}
	}
