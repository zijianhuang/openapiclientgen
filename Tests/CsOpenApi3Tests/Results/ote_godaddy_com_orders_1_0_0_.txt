//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace MyNS
{
	using System;
	using System.Linq;
	using System.Collections.Generic;
	using System.Threading.Tasks;
	using System.Text.Json;
	using System.Text.Json.Serialization;
	using Fonlow.Net.Http;
	
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Address
	{
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="address1")]
		public string Address1 { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="address2")]
		public string Address2 { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="city")]
		public string City { get; set; }
		
		/// <summary>
		/// Two-letter ISO country code to be used as a hint for target region<br/><br/>
		///NOTE: These are sample values, there are many
		///<a href='http://www.iso.org/iso/country_codes.htm'>more</a>
		/// Required
		/// </summary>
		[System.ComponentModel.DefaultValue(AddressCountry.US)]
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="country")]
		public AddressCountry Country { get; set; } = AddressCountry.US;
		
		/// <summary>
		/// Postal or zip code
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="postalCode")]
		public string PostalCode { get; set; }
		
		/// <summary>
		/// State or province or territory
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="state")]
		public string State { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public enum AddressCountry
	{
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AC = 0,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AD = 1,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AE = 2,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AF = 3,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AG = 4,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AI = 5,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AL = 6,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AM = 7,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AO = 8,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AQ = 9,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AR = 10,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AS = 11,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AT = 12,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AU = 13,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AW = 14,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AX = 15,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AZ = 16,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BA = 17,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BB = 18,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BD = 19,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BE = 20,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BF = 21,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BG = 22,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BH = 23,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BI = 24,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BJ = 25,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BM = 26,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BN = 27,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BO = 28,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BQ = 29,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BR = 30,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BS = 31,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BT = 32,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BV = 33,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BW = 34,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BY = 35,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		BZ = 36,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CA = 37,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CC = 38,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CD = 39,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CF = 40,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CG = 41,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CH = 42,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CI = 43,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CK = 44,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CL = 45,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CM = 46,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CN = 47,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CO = 48,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CR = 49,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CV = 50,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CW = 51,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CX = 52,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CY = 53,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CZ = 54,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DE = 55,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DJ = 56,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DK = 57,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DM = 58,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DO = 59,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		DZ = 60,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		EC = 61,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		EE = 62,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		EG = 63,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		EH = 64,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ER = 65,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ES = 66,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ET = 67,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FI = 68,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FJ = 69,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FK = 70,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FM = 71,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FO = 72,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		FR = 73,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GA = 74,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GB = 75,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GD = 76,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GE = 77,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GF = 78,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GG = 79,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GH = 80,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GI = 81,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GL = 82,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GM = 83,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GN = 84,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GP = 85,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GQ = 86,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GR = 87,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GS = 88,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GT = 89,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GU = 90,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GW = 91,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GY = 92,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HK = 93,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HM = 94,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HN = 95,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HR = 96,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HT = 97,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		HU = 98,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ID = 99,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IE = 100,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IL = 101,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IM = 102,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IN = 103,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IO = 104,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IQ = 105,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IS = 106,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IT = 107,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		JE = 108,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		JM = 109,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		JO = 110,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		JP = 111,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KE = 112,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KG = 113,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KH = 114,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KI = 115,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KM = 116,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KN = 117,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KR = 118,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KV = 119,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KW = 120,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KY = 121,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		KZ = 122,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LA = 123,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LB = 124,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LC = 125,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LI = 126,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LK = 127,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LR = 128,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LS = 129,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LT = 130,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LU = 131,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LV = 132,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		LY = 133,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MA = 134,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MC = 135,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MD = 136,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ME = 137,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MG = 138,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MH = 139,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MK = 140,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ML = 141,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MM = 142,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MN = 143,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MO = 144,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MP = 145,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MQ = 146,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MR = 147,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MS = 148,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MT = 149,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MU = 150,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MV = 151,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MW = 152,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MX = 153,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MY = 154,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MZ = 155,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NA = 156,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NC = 157,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NE = 158,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NF = 159,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NG = 160,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NI = 161,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NL = 162,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NO = 163,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NP = 164,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NR = 165,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NU = 166,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		NZ = 167,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		OM = 168,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PA = 169,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PE = 170,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PF = 171,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PG = 172,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PH = 173,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PK = 174,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PL = 175,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PM = 176,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PN = 177,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PR = 178,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PS = 179,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PT = 180,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PW = 181,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PY = 182,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		QA = 183,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		RE = 184,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		RO = 185,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		RS = 186,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		RU = 187,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		RW = 188,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SA = 189,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SB = 190,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SC = 191,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SE = 192,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SG = 193,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SH = 194,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SI = 195,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SJ = 196,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SK = 197,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SL = 198,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SM = 199,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SN = 200,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SO = 201,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SR = 202,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ST = 203,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SV = 204,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SX = 205,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SZ = 206,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TC = 207,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TD = 208,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TF = 209,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TG = 210,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TH = 211,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TJ = 212,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TK = 213,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TL = 214,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TM = 215,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TN = 216,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TO = 217,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TP = 218,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TR = 219,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TT = 220,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TV = 221,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TW = 222,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		TZ = 223,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UA = 224,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UG = 225,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UM = 226,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		US = 227,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UY = 228,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UZ = 229,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VA = 230,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VC = 231,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VE = 232,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VG = 233,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VI = 234,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VN = 235,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VU = 236,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		WF = 237,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		WS = 238,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		YE = 239,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		YT = 240,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ZA = 241,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ZM = 242,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ZW = 243,
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class BillTo
	{
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="contact")]
		public Contact Contact { get; set; }
		
		/// <summary>
		/// Tax id used for calculating the tax the customer is required to pay
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="taxId")]
		public string TaxId { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Contact
	{
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="addressMailing")]
		public Address AddressMailing { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="email")]
		public string Email { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="fax")]
		public string Fax { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="jobTitle")]
		public string JobTitle { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="nameFirst")]
		public string NameFirst { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="nameLast")]
		public string NameLast { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="nameMiddle")]
		public string NameMiddle { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="organization")]
		public string Organization { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="phone")]
		public string Phone { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Error
	{
		
		/// <summary>
		/// Short identifier for the error, suitable for indicating the specific error within client code
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="code")]
		public string Code { get; set; }
		
		/// <summary>
		/// List of the specific fields, and the errors found with their contents
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="fields")]
		public ErrorField[] Fields { get; set; }
		
		/// <summary>
		/// Human-readable, English description of the error
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="message")]
		public string Message { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class ErrorField
	{
		
		/// <summary>
		/// Short identifier for the error, suitable for indicating the specific error within client code
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="code")]
		public string Code { get; set; }
		
		/// <summary>
		/// Human-readable, English description of the problem with the contents of the field
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="message")]
		public string Message { get; set; }
		
		/// <summary>
		/// <ul>
		///<li style='margin-left: 12px;'>JSONPath referring to a field containing an error</li>
		///<strong style='margin-left: 12px;'>OR</strong>
		///<li style='margin-left: 12px;'>JSONPath referring to a field that refers to an object containing an error, with more detail in `pathRelated`</li>
		///</ul>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="path")]
		public string Path { get; set; }
		
		/// <summary>
		/// JSONPath referring to a field containing an error, which is referenced by `path`
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="pathRelated")]
		public string PathRelated { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class ErrorLimit
	{
		
		/// <summary>
		/// Short identifier for the error, suitable for indicating the specific error within client code
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="code")]
		public string Code { get; set; }
		
		/// <summary>
		/// List of the specific fields, and the errors found with their contents
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="fields")]
		public ErrorField[] Fields { get; set; }
		
		/// <summary>
		/// Human-readable, English description of the error
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="message")]
		public string Message { get; set; }
		
		/// <summary>
		/// Number of seconds to wait before attempting a similar request
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="retryAfterSec")]
		public string RetryAfterSec { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItem
	{
		
		/// <summary>
		/// A collection of domain names purchased if the current product is domain
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="domains")]
		public string[] Domains { get; set; }
		
		/// <summary>
		/// Human readable description of the current product
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="label")]
		public string Label { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="period")]
		public System.Nullable<System.Double> Period { get; set; }
		
		/// <summary>
		/// The unit of time that periodCount is measured in
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="periodUnit")]
		public System.Nullable<LineItemPeriodUnit> PeriodUnit { get; set; }
		
		/// <summary>
		/// Unique identifier of the current product
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="pfid")]
		public System.Nullable<System.Int32> Pfid { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="pricing")]
		public LineItemPricing Pricing { get; set; }
		
		/// <summary>
		/// Number of the current product included in the specified order
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="quantity")]
		public string Quantity { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="taxCollector")]
		public LineItemTaxCollector TaxCollector { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public enum LineItemPeriodUnit
	{
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MONTH = 0,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		QUARTER = 1,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		SEMI_ANNUAL = 2,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		YEAR = 3,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ONE_TIME = 4,
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItemPricing
	{
		
		/// <summary>
		/// Discount off of sale price for given `quantity` and `period` <pre>`discount` = `sale` - `subtotal`</pre>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="discount")]
		public string Discount { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="fees")]
		public OrderFee Fees { get; set; }
		
		/// <summary>
		/// List price for given `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="list")]
		public string List { get; set; }
		
		/// <summary>
		/// Actual price for the current product
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="sale")]
		public string Sale { get; set; }
		
		/// <summary>
		/// Savings off of list price for given `quantity` and `period` <pre>`savings` = `list` - `subtotal`</pre>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="savings")]
		public string Savings { get; set; }
		
		/// <summary>
		/// Price with any discounts and without taxes or fees for given `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="subtotal")]
		public string Subtotal { get; set; }
		
		/// <summary>
		/// Taxes for given `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="taxes")]
		public string Taxes { get; set; }
		
		/// <summary>
		/// Pricing for a single unit of the given item
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="unit")]
		public string Unit { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItemPricingTaxDetail
	{
		
		/// <summary>
		/// Tax amount
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="amount")]
		public string Amount { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="rate")]
		public System.Nullable<System.Double> Rate { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItemSummary
	{
		
		/// <summary>
		/// Product label of the current line item
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="label")]
		public string Label { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItemTaxCollector
	{
		
		/// <summary>
		/// Unique identifier for the company
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="taxCollectorId")]
		public string TaxCollectorId { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class LineItemUnitPricing
	{
		
		/// <summary>
		/// Discount without consideration of `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="discount")]
		public string Discount { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="fees")]
		public OrderFee Fees { get; set; }
		
		/// <summary>
		/// List price without consideration of `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="list")]
		public string List { get; set; }
		
		/// <summary>
		/// Sale price without consideration of `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="sale")]
		public string Sale { get; set; }
		
		/// <summary>
		/// Savings without consideration of `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="savings")]
		public string Savings { get; set; }
		
		/// <summary>
		/// Taxes without consideration of `quantity` and `period`
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="taxes")]
		public string Taxes { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Order
	{
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="billTo")]
		public BillTo BillTo { get; set; }
		
		/// <summary>
		/// Date and time when the current order is created on
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="createdAt")]
		public string CreatedAt { get; set; }
		
		/// <summary>
		/// Currency in which the order has been placed
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="currency")]
		public string Currency { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="items")]
		public LineItem[] Items { get; set; }
		
		/// <summary>
		/// Unique identifier of current order
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="orderId")]
		public string OrderId { get; set; }
		
		/// <summary>
		/// Unique identifier of the parent order. All refund/chargeback orders are tied to the original order.
		///The orginal order's `orderId` is the `parentOrderId` of refund/chargeback orders
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="parentOrderId")]
		public string ParentOrderId { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="payments")]
		public Payment[] Payments { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="pricing")]
		public OrderPricing Pricing { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Payment
	{
		
		/// <summary>
		/// Amount paid by the current payment
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="amount")]
		public string Amount { get; set; }
		
		/// <summary>
		/// The category of payment method used at the time of purchase.
		///Most common values are listed in the enum.
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="category")]
		public PaymentCategory Category { get; set; }
		
		/// <summary>
		/// Unique identifier of the payment profile used by current payment
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="paymentProfileId")]
		public string PaymentProfileId { get; set; }
		
		/// <summary>
		/// The subcategory of payment method used at the time of purchase.
		/// This field is only set for certain payment categories, such as ACH and CREDIT_CARD.
		///Most common values are listed in the enum.
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="subcategory")]
		public System.Nullable<PaymentSubcategory> Subcategory { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public enum PaymentCategory
	{
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CREDIT_CARD = 0,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PAYPAL = 1,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		ACH = 2,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		GIFT_CARD = 3,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		IN_STORE_CREDIT = 4,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		PREPAID = 5,
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public enum PaymentSubcategory
	{
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CHECKING_PERSONAL = 0,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		CHECKING_BUSINESS = 1,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		VISA = 2,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		MASTER_CARD = 3,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		AMEX = 4,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		JCB = 5,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		UNIONPAY = 6,
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class OrderFee
	{
		
		/// <summary>
		/// Domain registration transaction fee charged by ICANN
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="icann")]
		public string Icann { get; set; }
		
		/// <summary>
		/// Sum of all fees
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="total")]
		public string Total { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class OrderList
	{
		
		/// <summary>
		/// A collection of orders
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="orders")]
		public OrderSummary[] Orders { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="pagination")]
		public Pagination Pagination { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class OrderSummary
	{
		
		/// <summary>
		/// Date and time when the current order was created
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="createdAt")]
		public string CreatedAt { get; set; }
		
		/// <summary>
		/// Currency in which the order was placed
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="currency")]
		public string Currency { get; set; }
		
		/// <summary>
		/// Sets of two or more line items in current order
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="items")]
		public LineItemSummary[] Items { get; set; }
		
		/// <summary>
		/// Unique identifier of the current order
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="orderId")]
		public string OrderId { get; set; }
		
		/// <summary>
		/// Unique identifier of the parent order. All refund/chargeback orders are tied to the original order.
		///The orginal order's `orderId` is the `parentOrderId` of refund/chargeback orders
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="parentOrderId")]
		public string ParentOrderId { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="pricing")]
		public OrderSummaryPricing Pricing { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class OrderPricing
	{
		
		/// <summary>
		/// Discount from promotional pricing
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="discount")]
		public string Discount { get; set; }
		
		/// <summary>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="fees")]
		public OrderFee Fees { get; set; }
		
		[System.Runtime.Serialization.DataMember(Name="id")]
		public System.Nullable<System.Double> Id { get; set; }
		
		/// <summary>
		/// Sum of list prices for the entire cart
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="list")]
		public string List { get; set; }
		
		/// <summary>
		/// Savings off of list price <pre>`savings` = `list` - `subtotal`</pre>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="savings")]
		public string Savings { get; set; }
		
		/// <summary>
		/// Price with `discount` and without taxes or fees
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="subtotal")]
		public string Subtotal { get; set; }
		
		/// <summary>
		/// A collection of line item tax details
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="taxDetails")]
		public LineItemPricingTaxDetail[] TaxDetails { get; set; }
		
		/// <summary>
		/// Taxes for the entire cart
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="taxes")]
		public string Taxes { get; set; }
		
		/// <summary>
		/// Price the customer pays <pre>`total` = `subtotal` + `taxes` + `fees.total`</pre>
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="total")]
		public string Total { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class OrderSummaryPricing
	{
		
		/// <summary>
		/// Total amount charged for the current order. Negative for Refund and Chargeback
		/// Required
		/// </summary>
		[System.ComponentModel.DataAnnotations.Required()]
		[System.Runtime.Serialization.DataMember(Name="total")]
		public string Total { get; set; }
	}
	
	[System.Runtime.Serialization.DataContract(Namespace="http://fonlow.com/TestOpenApi/2024/04")]
	public class Pagination
	{
		
		/// <summary>
		/// URI to access the first page
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="first")]
		public string First { get; set; }
		
		/// <summary>
		/// URI to access the last page
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="last")]
		public string Last { get; set; }
		
		/// <summary>
		/// URI to access the next page
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="next")]
		public string Next { get; set; }
		
		/// <summary>
		/// URI to access the previous page
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="previous")]
		public string Previous { get; set; }
		
		/// <summary>
		/// Number of records available
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="total")]
		public System.Nullable<System.Int32> Total { get; set; }
	}
	
	public partial class Misc
	{
		
		private System.Net.Http.HttpClient httpClient;
		
		private JsonSerializerOptions jsonSerializerSettings;
		
		public Misc(System.Net.Http.HttpClient httpClient, JsonSerializerOptions jsonSerializerSettings=null)
		{
			if (httpClient == null)
				throw new ArgumentNullException("Null HttpClient.", "httpClient");

			if (httpClient.BaseAddress == null)
				throw new ArgumentNullException("HttpClient has no BaseAddress", "httpClient");

			this.httpClient = httpClient;
			this.jsonSerializerSettings = jsonSerializerSettings;
		}
		
		/// <summary>
		/// Retrieve a list of orders for the authenticated shopper. Only one filter may be used at a time
		/// <strong>API Resellers</strong><ul><li>This endpoint does not support subaccounts and therefore API Resellers should not supply an X-Shopper-Id header</li></ul>
		/// List v1/orders
		/// </summary>
		/// <param name="periodStart">Start of range indicating what time-frame should be returned. Inclusive</param>
		/// <param name="periodEnd">End of range indicating what time-frame should be returned. Inclusive</param>
		/// <param name="domain">Domain name to use as the filter of results</param>
		/// <param name="productGroupId">Product group id to use as the filter of results</param>
		/// <param name="paymentProfileId">Payment profile id to use as the filter of results</param>
		/// <param name="parentOrderId">Parent order id to use as the filter of results</param>
		/// <param name="offset">Number of results to skip for pagination</param>
		/// <param name="limit">Maximum number of items to return
		/// Minimum: 1
		/// Maximum: 10000
		// </param>
		/// <param name="sort">Property name that will be used to sort results. '-' indicates descending</param>
		/// <returns>Request was successful</returns>
		public async Task<OrderList> ListAsync(string periodStart, string periodEnd, string domain, int productGroupId, int paymentProfileId, string parentOrderId, int offset, int limit, ListSort sort, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "v1/orders?periodStart=" + (periodStart==null? "" : System.Uri.EscapeDataString(periodStart))+"&periodEnd=" + (periodEnd==null? "" : System.Uri.EscapeDataString(periodEnd))+"&domain=" + (domain==null? "" : System.Uri.EscapeDataString(domain))+"&productGroupId="+productGroupId+"&paymentProfileId="+paymentProfileId+"&parentOrderId=" + (parentOrderId==null? "" : System.Uri.EscapeDataString(parentOrderId))+"&offset="+offset+"&limit="+limit+"&sort=" + sort;
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			if (handleHeaders != null)
			{
				handleHeaders(httpRequestMessage.Headers);
			}

			var responseMessage = await httpClient.SendAsync(httpRequestMessage);
			try
			{
				responseMessage.EnsureSuccessStatusCodeEx();
				var streamContent = await responseMessage.Content.ReadAsStreamAsync();
				return JsonSerializer.Deserialize<OrderList>(streamContent, jsonSerializerSettings);
			}
			finally
			{
				responseMessage.Dispose();
			}
		}
		
		/// <summary>
		/// Retrieve details for specified order
		/// <strong>API Resellers</strong><ul><li>This endpoint does not support subaccounts and therefore API Resellers should not supply an X-Shopper-Id header</li></ul>
		/// Get v1/orders/{orderId}
		/// </summary>
		/// <param name="orderId">Order id whose details are to be retrieved</param>
		/// <returns>Request was successful</returns>
		public async Task<Order> GetAsync(string orderId, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "v1/orders/"+ (orderId==null? "" : System.Uri.EscapeDataString(orderId));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			if (handleHeaders != null)
			{
				handleHeaders(httpRequestMessage.Headers);
			}

			var responseMessage = await httpClient.SendAsync(httpRequestMessage);
			try
			{
				responseMessage.EnsureSuccessStatusCodeEx();
				var streamContent = await responseMessage.Content.ReadAsStreamAsync();
				return JsonSerializer.Deserialize<Order>(streamContent, jsonSerializerSettings);
			}
			finally
			{
				responseMessage.Dispose();
			}
		}
	}
	
	public enum ListSort
	{
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		createdAt = 0,
		
		[System.Runtime.Serialization.EnumMemberAttribute(Value="-createdAt")]
		MinuscreatedAt = 1,
		
		[System.Runtime.Serialization.EnumMemberAttribute()]
		orderId = 2,
		
		[System.Runtime.Serialization.EnumMemberAttribute(Value="-orderId")]
		MinusorderId = 3,
		
		[System.Runtime.Serialization.EnumMemberAttribute(Value="pricing.total")]
		pricing_total = 4,
		
		[System.Runtime.Serialization.EnumMemberAttribute(Value="-pricing.total")]
		Minuspricing_total = 5,
	}
}

namespace Fonlow.Net.Http
{
	using System.Net.Http;

	public class WebApiRequestException : HttpRequestException
	{
		public new System.Net.HttpStatusCode StatusCode { get; private set; }

		public string Response { get; private set; }

		public System.Net.Http.Headers.HttpResponseHeaders Headers { get; private set; }

		public System.Net.Http.Headers.MediaTypeHeaderValue ContentType { get; private set; }

		public WebApiRequestException(string message, System.Net.HttpStatusCode statusCode, string response, System.Net.Http.Headers.HttpResponseHeaders headers, System.Net.Http.Headers.MediaTypeHeaderValue contentType) : base(message)
		{
			StatusCode = statusCode;
			Response = response;
			Headers = headers;
			ContentType = contentType;
		}
	}

	public static class ResponseMessageExtensions
	{
		public static void EnsureSuccessStatusCodeEx(this HttpResponseMessage responseMessage)
		{
			if (!responseMessage.IsSuccessStatusCode)
			{
				var responseText = responseMessage.Content.ReadAsStringAsync().Result;
				var contentType = responseMessage.Content.Headers.ContentType;
				throw new WebApiRequestException(responseMessage.ReasonPhrase, responseMessage.StatusCode, responseText, responseMessage.Headers, contentType);
			}
		}
	}
}
