...
namespace MyNS
{
	...
	public class GoogleCloudPolicyanalyzerV1Activity
	{
		...
		public GoogleCloudPolicyanalyzerV1ObservationPeriod ObservationPeriod { get; set; }
	}
	...
	public class GoogleCloudPolicyanalyzerV1QueryActivityResponse
	{
		...
		public GoogleCloudPolicyanalyzerV1Activity[] Activities { get; set; }
	...
	public partial class Misc
	{
		...
		/// Policyanalyzer_projects_locations_activityTypes_activities_query v1/{parent}/activities:query
		/// </summary>
		/// <param name="parent">Required. The container resource on which to execute the request. Acceptable formats: `projects/[PROJECT_ID|PROJECT_NUMBER]/locations/[LOCATION]/activityTypes/[ACTIVITY_TYPE]` LOCATION here refers to Google Cloud Locations: https://cloud.google.com/about/locations/</param>
		/// <param name="filter">Optional. Filter expression to restrict the activities returned. For serviceAccountLastAuthentication activities, supported filters are: - `activities.full_resource_name {=} [STRING]` - `activities.fullResourceName {=} [STRING]` where `[STRING]` is the full resource name of the service account. For serviceAccountKeyLastAuthentication activities, supported filters are: - `activities.full_resource_name {=} [STRING]` - `activities.fullResourceName {=} [STRING]` where `[STRING]` is the full resource name of the service account key.</param>
		...
		public async Task<GoogleCloudPolicyanalyzerV1QueryActivityResponse> Policyanalyzer_projects_locations_activityTypes_activities_queryAsync(string parent, string filter, int pageSize, string pageToken, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "v1/"+ (parent==null? "" : System.Uri.EscapeDataString(parent))+"/activities:query&filter=" + (filter==null? "" : System.Uri.EscapeDataString(filter))+"&pageSize="+pageSize+"&pageToken=" + (pageToken==null? "" : System.Uri.EscapeDataString(pageToken));
			...
			try
			{
				...
				return JsonSerializer.Deserialize<GoogleCloudPolicyanalyzerV1QueryActivityResponse>(streamContent, jsonSerializerSettings);
			}
			finally
			{
