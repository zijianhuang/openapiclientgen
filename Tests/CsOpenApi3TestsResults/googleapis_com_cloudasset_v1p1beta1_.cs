...
namespace MyNS
{
	...
	public class SearchAllResourcesResponse
	{
		...
		/// </summary>
		[System.Runtime.Serialization.DataMember(Name="results")]
		public StandardResourceMetadata[] Results { get; set; }
	}
	...
	public partial class Misc
	{
		...
		/// Cloudasset_iamPolicies_searchAll v1p1beta1/{scope}/iamPolicies:searchAll
		/// </summary>
		/// <param name="scope">Required. The relative name of an asset. The search is limited to the resources within the `scope`. The allowed value must be: * Organization number (such as "organizations/123") * Folder number (such as "folders/1234") * Project number (such as "projects/12345") * Project ID (such as "projects/abc")</param>
		...
		/// <param name="query">Optional. The query statement. Examples: * "policy:myuser@mydomain.com" * "policy:(myuser@mydomain.com viewer)"</param>
		/// <returns>Successful response</returns>
		public async Task<SearchAllIamPoliciesResponse> Cloudasset_iamPolicies_searchAllAsync(string scope, int pageSize, string pageToken, string query, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "v1p1beta1/"+ (scope==null? "" : System.Uri.EscapeDataString(scope))+"/iamPolicies:searchAll&pageSize="+pageSize+"&pageToken=" + (pageToken==null? "" : System.Uri.EscapeDataString(pageToken))+"&query=" + (query==null? "" : System.Uri.EscapeDataString(query));
		...
		/// Cloudasset_resources_searchAll v1p1beta1/{scope}/resources:searchAll
		/// </summary>
		/// <param name="scope">Required. The relative name of an asset. The search is limited to the resources within the `scope`. The allowed value must be: * Organization number (such as "organizations/123") * Folder number (such as "folders/1234") * Project number (such as "projects/12345") * Project ID (such as "projects/abc")</param>
		/// <param name="assetTypes">Optional. A list of asset types that this request searches for. If empty, it will search all the supported asset types.</param>
		/// <param name="orderBy">Optional. A comma separated list of fields specifying the sorting order of the results. The default order is ascending. Add ` DESC` after the field name to indicate descending order. Redundant space characters are ignored. For example, ` location DESC , name `.</param>
		/// <param name="pageSize">Optional. The page size for search result pagination. Page size is capped at 500 even if a larger value is given. If set to zero, server will pick an appropriate default. Returned results may be fewer than requested. When this happens, there could be more results as long as `next_page_token` is returned.</param>
		...
		/// <param name="query">Optional. The query statement.</param>
		/// <returns>Successful response</returns>
		public async Task<SearchAllResourcesResponse> Cloudasset_resources_searchAllAsync(string scope, string[] assetTypes, string orderBy, int pageSize, string pageToken, string query, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "v1p1beta1/"+ (scope==null? "" : System.Uri.EscapeDataString(scope))+"/resources:searchAll&"+string.Join("&", assetTypes.Select(z => $"assetTypes={System.Uri.EscapeDataString(z.ToString())}"))+"&orderBy=" + (orderBy==null? "" : System.Uri.EscapeDataString(orderBy))+"&pageSize="+pageSize+"&pageToken=" + (pageToken==null? "" : System.Uri.EscapeDataString(pageToken))+"&query=" + (query==null? "" : System.Uri.EscapeDataString(query));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			if (handleHeaders != null)
			{
				handleHeaders(httpRequestMessage.Headers);
