...
namespace MyNS
{
	...
	public partial class Misc
	{
		...
		public async Task<ImageAnalysis> AnalyzeImageAsync(string[] visualFeatures, string[] details, AnalyzeImageLanguage language, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
		...
		public async Task<AreaOfInterestResult> GetAreaOfInterestAsync(ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			...
			try
			{
				responseMessage.EnsureSuccessStatusCodeEx();
				var streamContent = await responseMessage.Content.ReadAsStreamAsync();
				return JsonSerializer.Deserialize<AreaOfInterestResult>(streamContent, jsonSerializerSettings);
			}
			finally
			{
				responseMessage.Dispose();
		...
		public async Task<ImageDescription> DescribeImageAsync(int maxCandidates, AnalyzeImageLanguage language, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			var requestUri = "describe?maxCandidates="+maxCandidates+"&language=" + language;
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Post, requestUri);
			var content = System.Net.Http.Json.JsonContent.Create(requestBody, mediaType: null, jsonSerializerSettings);
			httpRequestMessage.Content = content;
			if (handleHeaders != null)
		...
		public async Task<DetectResult> DetectObjectsAsync(ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
			...
			try
			{
				responseMessage.EnsureSuccessStatusCodeEx();
				var streamContent = await responseMessage.Content.ReadAsStreamAsync();
				return JsonSerializer.Deserialize<DetectResult>(streamContent, jsonSerializerSettings);
			}
			finally
			{
				responseMessage.Dispose();
		...
		public async Task GenerateThumbnailAsync(int width, int height, bool smartCropping, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
		...
		public async Task<ListModelsResult> ListModelsAsync(Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
		...
		public async Task<DomainModelResults> AnalyzeImageByDomainAsync(string model, AnalyzeImageLanguage language, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
		...
		/// <param name="requestBody">A JSON document with a URL pointing to the image that is to be analyzed.</param>
		/// <returns>The OCR results in the hierarchy of region/line/word. The results include text, bounding box for regions, lines and words. The angle, in degrees, of the detected text with respect to the closest horizontal or vertical direction. After rotating the input image clockwise by this angle, the recognized text lines become horizontal or vertical. In combination with the orientation property it can be used to overlay recognition results correctly on the original image, by rotating either the original image or recognition results by a suitable angle around the center of the original image. If the angle cannot be confidently detected, this property is not present. If the image contains text at different angles, only part of the text will be recognized correctly.</returns>
		public async Task<OcrResult> RecognizePrintedTextAsync(bool detectOrientation, RecognizePrintedTextLanguage language, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
		...
		public async Task<TagResult> TagImageAsync(AnalyzeImageLanguage language, ImageUrl requestBody, Action<System.Net.Http.Headers.HttpRequestHeaders> handleHeaders = null)
		{
