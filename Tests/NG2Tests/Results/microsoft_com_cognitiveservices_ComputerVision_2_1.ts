...
export namespace MyNS {
	...
	export interface AdultInfoFormProperties {
		...
		/** A value indicating if the image is gory. */
		isGoryContent: FormControl<boolean | null | undefined>,
	...
	export function CreateAdultInfoFormGroup() {
		return new FormGroup<AdultInfoFormProperties>({
			...
			goreScore: new FormControl<number | null | undefined>(undefined),
			isAdultContent: new FormControl<boolean | null | undefined>(undefined),
			isGoryContent: new FormControl<boolean | null | undefined>(undefined),
	...
	export class MyClient {
		 ...
		 */
		AnalyzeImage(visualFeatures: Array<string> | null | undefined, details: Array<string> | null | undefined, language: AnalyzeImageLanguage | null | undefined, descriptionExclude: Array<string> | null | undefined, requestBody: ImageUrl): Observable<ImageAnalysis> {
			return this.http.post<ImageAnalysis>(this.baseUri + 'analyze?' + visualFeatures?.map(z => `visualFeatures=${encodeURIComponent(z)}`).join('&') + '&' + details?.map(z => `details=${encodeURIComponent(z)}`).join('&') + '&language=' + language + '&' + descriptionExclude?.map(z => `descriptionExclude=${encodeURIComponent(z)}`).join('&'), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 */
		DescribeImage(maxCandidates: number | null | undefined, language: AnalyzeImageLanguage | null | undefined, descriptionExclude: Array<string> | null | undefined, requestBody: ImageUrl): Observable<ImageDescription> {
			return this.http.post<ImageDescription>(this.baseUri + 'describe?maxCandidates=' + maxCandidates + '&language=' + language + '&' + descriptionExclude?.map(z => `descriptionExclude=${encodeURIComponent(z)}`).join('&'), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 * @param {ImageUrl} requestBody A JSON document with a URL pointing to the image that is to be analyzed.
		 * @return {OcrResult} The OCR results in the hierarchy of region/line/word. The results include text, bounding box for regions, lines and words. The angle, in radians, of the detected text with respect to the closest horizontal or vertical direction. After rotating the input image clockwise by this angle, the recognized text lines become horizontal or vertical. In combination with the orientation property it can be used to overlay recognition results correctly on the original image, by rotating either the original image or recognition results by a suitable angle around the center of the original image. If the angle cannot be confidently detected, this property is not present. If the image contains text at different angles, only part of the text will be recognized correctly.
		 */
