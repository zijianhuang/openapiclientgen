...
export namespace MyNS {
	...
	export interface StepPropertiesFormProperties {
		 ...
		 * Required
		 */
		stepType: FormControl<string | null | undefined>,
	}
	export function CreateStepPropertiesFormGroup() {
		return new FormGroup<StepPropertiesFormProperties>({
			stepType: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});
	...
	export class MyClient {
		 ...
		 * @return {Array<Operation>} The operation details are successfully returned. The response code does not determine if the request which this operation corresponds to is successful. Check the response content for the status and any error information on failures.
		 */
		Operations_Get(subscriptionId: string, api_version: string): Observable<Array<Operation>> {
			return this.http.get<Array<Operation>>(this.baseUri + 'subscriptions/' + (subscriptionId == null ? '' : encodeURIComponent(subscriptionId)) + '/providers/Microsoft.DeploymentManager/operations&api_version=' + (api_version == null ? '' : encodeURIComponent(api_version)), {});
		}
