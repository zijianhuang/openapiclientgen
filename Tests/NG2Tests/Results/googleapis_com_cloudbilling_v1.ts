...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {string} filter Options for how to filter the returned billing accounts.
		 * Currently this only supports filtering for
		 * [subaccounts](https://cloud.google.com/billing/docs/concepts) under a
		 * single provided reseller billing account.
		 * (e.g. "master_billing_account=billingAccounts/012345-678901-ABCDEF").
		 * Boolean algebra and other fields are not currently supported.
		 * @param {number} pageSize Requested page size. The maximum page size is 100; this is also the
		 * default.
		 * @param {string} pageToken A token identifying a page of results to return. This should be a
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_billingAccounts_create(requestBody: BillingAccount): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'v1/billingAccounts', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_billingAccounts_get(name: string): Observable<HttpResponse<string>> {
		 ...
		 * @param {string} updateMask The update mask applied to the resource.
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_projects_getBillingInfo(name: string): Observable<HttpResponse<string>> {
			return this.http.get(this.baseUri + 'v1/' + (name == null ? '' : encodeURIComponent(name)) + '/billingInfo', { observe: 'response', responseType: 'text' });
		}
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_projects_updateBillingInfo(name: string, requestBody: ProjectBillingInfo): Observable<HttpResponse<string>> {
			return this.http.put(this.baseUri + 'v1/' + (name == null ? '' : encodeURIComponent(name)) + '/billingInfo', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
		 ...
		 * Example: "services/DA34-426B-A397"
		 * @param {string} currencyCode The ISO 4217 currency code for the pricing info in the response proto.
		 * Will use the conversion rate as of start_time.
		 * Optional. If not specified USD will be used.
		 * @param {string} endTime Optional exclusive end time of the time range for which the pricing
		 ...
		 * call. If unspecified, the first page of results is returned.
		 * @param {string} startTime Optional inclusive start time of the time range for which the pricing
		 * versions will be returned. Timestamps in the future are not allowed.
		 * The time range has to be within a single calendar month in
		 * America/Los_Angeles timezone. Time range as a whole is optional. If not
		 * specified, the latest pricing will be returned (up to 12 hours old at
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_billingAccounts_setIamPolicy(resource: string, requestBody: SetIamPolicyRequest): Observable<HttpResponse<string>> {
		 ...
		 * @return {void} Successful response
		 */
		Cloudbilling_billingAccounts_testIamPermissions(resource: string, requestBody: TestIamPermissionsRequest): Observable<HttpResponse<string>> {
