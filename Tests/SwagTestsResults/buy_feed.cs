...
namespace MyNS
{
	...
	public class Item
	{
		...
		/// <summary>
		/// The unique identifier of an item in eBay RESTful format. An example would be v1|162846450672|461882996982.
		/// </summary>
		...
		/// <summary>
		/// The text describing the condition of the item. For a list of condition names, see Item Condition IDs and Names. Code so that your app gracefully handles any future changes to this list.
		/// </summary>
		...
		/// <summary>
		/// An identifier generated/incremented when a seller revises the item. There are two types of item revisions: Seller changes, such as changing the title eBay system changes, such as changing the quantity when an item is purchased This ID is changed only when the seller makes a change to the item.
		/// </summary>
		...
		/// <summary>
		/// The ePID (eBay Product ID of a product in the eBay product catalog) for the item, which has been programmatically determined by eBay using the item's title, aspects, and other data. If the seller actually provided an ePID at listing time for the item, the ePID value is returned in the epid column instead.
		/// </summary>
		...
		/// <summary>
		/// This is the price per unit for the item. Some European countries require listings for certain types of products to include the price per unit so buyers can accurately compare prices. For example: &quot;unitPricingMeasure&quot;: &quot;100g&quot;, &quot;unitPrice&quot;: { &nbsp;&nbsp;&quot;value&quot;: &quot;7.99&quot;, &nbsp;&nbsp;&quot;currency&quot;: &quot;GBP&quot;
		/// </summary>
	...
	public class ItemGroup
	{
		...
		/// <summary>
		/// A pipe separated (|) list of the aspect (variation) names for this item group. The aspect name is BASE64 encoded. Note: This column can contain multiple values. &nbsp;&nbsp; Encoded Format: &nbsp;&nbsp;&nbsp;aspectName|aspectName &nbsp;&nbsp; Encoded Example (The delimiters are empathized): &nbsp;&nbsp;&nbsp;Q29sb3I=|U2l6ZQ== &nbsp;&nbsp; Decoded: &nbsp;&nbsp;&nbsp;Color|Size
		/// </summary>
	...
	/// <summary>
	/// The type that defines the array for the items returned in the Item Group feed file.
	/// </summary>
	...
	/// <summary>
	/// The type that defines the columns returned in the Hourly Snapshot feed file. Note: When the value of the availability column is UNAVAILABLE, only the itemId and availability columns are populated.
	/// </summary>
	public class ItemSnapshot
	{
		...
		/// <summary>
		/// A timestamp indicating when the item's sale period will end based on its start date and duration. For Good 'Tail Cancelled items, no value is returned in this column. Format: UTC (yyyy-MM-ddThh:mm:ss.sssZ).
		/// </summary>
		public string ItemEndDate { get; set; }
		...
		/// <summary>
		/// An enumeration value representing how a buyer is refunded when an item is returned. Code so that your app gracefully handles any future changes to this list. For implementation help, refer to <a href='https://developer.ebay.com/devzone/rest/api-ref/feed/types/RefundMethodEnum.html'>eBay API documentation</a>
		/// </summary>
		public string RefundMethod { get; set; }
		...
		/// <summary>
		/// A timestamp indicating when the item was created. The format is yyyy-mm-dd.
		/// </summary>
	...
	/// <summary>
	/// The type that defines the array for the items returned in the Hourly Snapshot feed file.
	/// </summary>
	public class ItemSnapshotResponse
	{
		
		/// <summary>
		/// The container for the array of items returned by the getItemSnapshotFeed method. Note: When the value of the availability column is UNAVAILABLE, only the itemId and availability columns are populated.
		/// </summary>
	...
	public partial class MyClient
	{
		...
		public async Task GetItemFeedAsync(string feed_scope, string category_id, string date)
		{
		...
		/// <param name="date">The date of the daily Item feed file (feed_scope=NEWLY_LISTED) you want. The date is required only for the daily Item feed file. If you specify a date for the Item Bootstrap file (feed_scope=ALL_ACTIVE), the date is ignored and the latest file is returned. The date the Item Bootstrap feed file was generated is returned in the Last-Modified response header. The Item feed files are generated every day and there are 14 daily files available. Note: The daily Item feed files are available each day after 9AM MST (US Mountain Standard Time), which is -7 hours UTC time. There is a 48 hour latency when generating the Item feed files. This means you can download the file for July 10th on July 12 after 9AM MST. Note: For categories with a large number of items, the latency can be up to 72 hours. Format: yyyyMMdd Requirements: Required when feed_scope=NEWLY_LISTED Must be within 3-14 days in the past</param>
		/// <returns>Success</returns>
		public void GetItemFeed(string feed_scope, string category_id, string date)
		{
			var requestUri = "item?feed_scope=" + (feed_scope==null? "" : System.Uri.EscapeDataString(feed_scope))+"&category_id=" + (category_id==null? "" : System.Uri.EscapeDataString(category_id))+"&date=" + (date==null? "" : System.Uri.EscapeDataString(date));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
		...
		/// <returns>Success</returns>
		public async Task GetItemGroupFeedAsync(string feed_scope, string category_id, string date)
		{
		...
		/// <param name="date">The date of the daily Item Group feed file (feed_scope=NEWLY_LISTED) you want. The date is required only for the daily Item Group feed file. If you specify a date for the Item Group Bootstrap file (feed_scope=ALL_ACTIVE), the date is ignored and the latest file is returned. The date the Item Group Bootstrap feed file was generated is returned in the Last-Modified response header. The Item Group feed files are generated every day and there are 14 daily files available. There is a 48 hour latency when generating the files. This means on July 10, the latest feed file you can download is July 8. Note: The generated files are stored using MST (US Mountain Standard Time), which is -7 hours UTC time. Format: yyyyMMdd Requirement: Requirements: Required only when feed_scope=NEWLY_LISTED Must be within 3-14 days in the past</param>
		/// <returns>Success</returns>
		public void GetItemGroupFeed(string feed_scope, string category_id, string date)
		{
			var requestUri = "item_group?feed_scope=" + (feed_scope==null? "" : System.Uri.EscapeDataString(feed_scope))+"&category_id=" + (category_id==null? "" : System.Uri.EscapeDataString(category_id))+"&date=" + (date==null? "" : System.Uri.EscapeDataString(date));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			var responseMessage = httpClient.Send(httpRequestMessage);
		...
		/// <param name="snapshot_date">The date and hour of the snapshot feed file you want. Each file contains the items that changed within the hour in the specified category. So, the 9AM file contains the items that changed between 9AM and 9:59AM on the day specified. It takes 2 hours to generate a snapshot file, which means to get the file for 9AM the earliest you could submit the call is at 11AM. There are 7 days of Hourly Snapshot feed files available. Note: The Feed API uses GMT, so you must convert your local time to GMT. For example, if you lived in California and wanted the September 15th 7pm file, you would submit the following call: item_snapshot?category_id=625&amp;snapshot_date=2017-09-16T02:00:00.000Z Format: UTC format (yyyy-MM-ddThh:00:00.000Z) Files are generated on the hour, so minutes and seconds are always zeros.</param>
		/// <returns>Success</returns>
		public void GetItemSnapshotFeed(string category_id, string snapshot_date)
		{
			var requestUri = "item_snapshot?category_id=" + (category_id==null? "" : System.Uri.EscapeDataString(category_id))+"&snapshot_date=" + (snapshot_date==null? "" : System.Uri.EscapeDataString(snapshot_date));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			var responseMessage = httpClient.Send(httpRequestMessage);
