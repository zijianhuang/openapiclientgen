...
namespace MyNS
{
	...
	public partial class MyClient
	{
		...
		public async Task<GetCustomerServiceMetricResponse> GetCustomerServiceMetricAsync(string customer_service_metric_type, string evaluation_marketplace_id, string evaluation_type)
		{
			...
			try
			{
				...
				return serializer.Deserialize<GetCustomerServiceMetricResponse>(jsonReader);
			}
			finally
			{
		...
		/// <param name="evaluation_type">Use this path parameter to specify the type of the seller evaluation you want returned, either: CURRENT &ndash; A monthly evaluation that occurs on the 20th of every month. PROJECTED &ndash; A daily evaluation that provides a projection of how the seller is currently performing with regards to the upcoming evaluation period.</param>
		/// <returns>Success</returns>
		public GetCustomerServiceMetricResponse GetCustomerServiceMetric(string customer_service_metric_type, string evaluation_marketplace_id, string evaluation_type)
		{
			var requestUri = "customer_service_metric/"+ (customer_service_metric_type==null? "" : System.Uri.EscapeDataString(customer_service_metric_type))+"/"+ (evaluation_type==null? "" : System.Uri.EscapeDataString(evaluation_type))+"&evaluation_marketplace_id=" + (evaluation_marketplace_id==null? "" : System.Uri.EscapeDataString(evaluation_marketplace_id));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
		...
		/// </summary>
		/// <returns>Success</returns>
		public async Task<FindSellerStandardsProfilesResponse> FindSellerStandardsProfilesAsync()
		{
			...
			try
			{
				...
				using JsonReader jsonReader = new JsonTextReader(new System.IO.StreamReader(streamContent));
				var serializer = JsonSerializer.Create(jsonSerializerSettings);
				return serializer.Deserialize<FindSellerStandardsProfilesResponse>(jsonReader);
			}
			finally
			{
				responseMessage.Dispose();
		...
		/// <returns>Success</returns>
		public async Task<StandardsProfile> GetSellerStandardsProfileAsync(string cycle, string program)
		{
			...
			try
			{
				...
				using JsonReader jsonReader = new JsonTextReader(new System.IO.StreamReader(streamContent));
				var serializer = JsonSerializer.Create(jsonSerializerSettings);
				return serializer.Deserialize<StandardsProfile>(jsonReader);
			}
			finally
			{
				responseMessage.Dispose();
		...
		/// <returns>Success</returns>
		public async Task<Report> GetTrafficReportAsync(string dimension, string filter, string metric, string sort)
		{
		...
		/// <param name="sort">This query parameter sorts the report on the specified metric. The metric you specify must be included in the configuration of the report's metric parameter. Sorting is helpful when you want to review how a specific metric is performing, such as the CLICK_THROUGH_RATE. Reports can be sorted in ascending or descending order. Precede the value of a descending-order request with a minus sign (&quot;-&quot;), for example: sort=-CLICK_THROUGH_RATE. For implementation help, refer to eBay API documentation at https://developer.ebay.com/devzone/rest/api-ref/analytics/types/SortField.html</param>
		/// <returns>Success</returns>
		public Report GetTrafficReport(string dimension, string filter, string metric, string sort)
		{
			var requestUri = "traffic_report?dimension=" + (dimension==null? "" : System.Uri.EscapeDataString(dimension))+"&filter=" + (filter==null? "" : System.Uri.EscapeDataString(filter))+"&metric=" + (metric==null? "" : System.Uri.EscapeDataString(metric))+"&sort=" + (sort==null? "" : System.Uri.EscapeDataString(sort));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			var responseMessage = httpClient.Send(httpRequestMessage);
