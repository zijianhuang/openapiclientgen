...
namespace MyNS
{
	...
	public class CorrectiveRecommendations
	{
		...
		/// </summary>
		public AspectRecommendations[] AspectRecommendations { get; set; }
	}
	...
	public partial class MyClient
	{
		...
		/// GetListingViolationsSummary listing_violation_summary
		/// </summary>
		/// <param name="compliance_type">A user passes in one or more compliance type values through this query parameter. See ComplianceTypeEnum for more information on the supported compliance types that can be passed in here. If more than one compliance type value is used, delimit these values with a comma. If no compliance type values are passed in, the listing count for all compliance types will be returned. Note: Only a canned response, with counts for all listing compliance types, is returned in the Sandbox environment. Due to this limitation, the compliance_type query parameter (if used) will not have an effect on the response.</param>
		/// <returns>Success</returns>
		public async Task GetListingViolationsSummaryAsync(string compliance_type)
		{
			var requestUri = "listing_violation_summary?compliance_type=" + (compliance_type==null? "" : System.Uri.EscapeDataString(compliance_type));
		...
		/// <param name="compliance_type">A user passes in one or more compliance type values through this query parameter. See ComplianceTypeEnum for more information on the supported compliance types that can be passed in here. If more than one compliance type value is used, delimit these values with a comma. If no compliance type values are passed in, the listing count for all compliance types will be returned. Note: Only a canned response, with counts for all listing compliance types, is returned in the Sandbox environment. Due to this limitation, the compliance_type query parameter (if used) will not have an effect on the response.</param>
		/// <returns>Success</returns>
		public void GetListingViolationsSummary(string compliance_type)
		{
			var requestUri = "listing_violation_summary?compliance_type=" + (compliance_type==null? "" : System.Uri.EscapeDataString(compliance_type));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
		...
		/// GetListingViolations listing_violation
		/// </summary>
		...
		/// <param name="limit">This query parameter is used if the user wants to set a limit on the number of listing violations that are returned on one page of the result set. This parameter is used in conjunction with the offset parameter to control the pagination of the output. For example, if offset is set to 10 and limit is set to 10, the call retrieves listing violations 11 thru 20 from the collection of listing violations that match the value set in the compliance_type parameter. Note: This feature employs a zero-based index, where the first item in the list has an offset of 0. If the listing_id parameter is included in the request, this parameter will be ignored. Default: 100 Maximum: 200</param>
		/// <param name="filter">This filter allows a user to retrieve only listings that are currently out of compliance, or only listings that are at risk of becoming out of compliance. Although other filters may be added in the future, complianceState is the only supported filter type at this time. The two compliance 'states' are OUT_OF_COMPLIANCE and AT_RISK. Below is an example of how to set up this compliance state filter. Notice that the filter type and filter value are separated with a colon (:) character, and the filter value is wrapped with curly brackets. filter=complianceState:{OUT_OF_COMPLIANCE}</param>
		/// <returns>Success</returns>
		public async Task GetListingViolationsAsync(string compliance_type, string offset, string listing_id, string limit, string filter)
		{
			var requestUri = "listing_violation?compliance_type=" + (compliance_type==null? "" : System.Uri.EscapeDataString(compliance_type))+"&offset=" + (offset==null? "" : System.Uri.EscapeDataString(offset))+"&listing_id=" + (listing_id==null? "" : System.Uri.EscapeDataString(listing_id))+"&limit=" + (limit==null? "" : System.Uri.EscapeDataString(limit))+"&filter=" + (filter==null? "" : System.Uri.EscapeDataString(filter));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			var responseMessage = await httpClient.SendAsync(httpRequestMessage);
			try
			{
		...
		/// <param name="filter">This filter allows a user to retrieve only listings that are currently out of compliance, or only listings that are at risk of becoming out of compliance. Although other filters may be added in the future, complianceState is the only supported filter type at this time. The two compliance 'states' are OUT_OF_COMPLIANCE and AT_RISK. Below is an example of how to set up this compliance state filter. Notice that the filter type and filter value are separated with a colon (:) character, and the filter value is wrapped with curly brackets. filter=complianceState:{OUT_OF_COMPLIANCE}</param>
		/// <returns>Success</returns>
		public void GetListingViolations(string compliance_type, string offset, string listing_id, string limit, string filter)
		{
			var requestUri = "listing_violation?compliance_type=" + (compliance_type==null? "" : System.Uri.EscapeDataString(compliance_type))+"&offset=" + (offset==null? "" : System.Uri.EscapeDataString(offset))+"&listing_id=" + (listing_id==null? "" : System.Uri.EscapeDataString(listing_id))+"&limit=" + (limit==null? "" : System.Uri.EscapeDataString(limit))+"&filter=" + (filter==null? "" : System.Uri.EscapeDataString(filter));
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
			var responseMessage = httpClient.SendAsync(httpRequestMessage).Result;
		...
		/// SuppressViolation suppress_listing_violation
		/// </summary>
		/// <param name="requestBody">This type is the base request type of the SuppressViolation method.</param>
		public async Task SuppressViolationAsync(SuppressViolationRequest requestBody)
		{
			var requestUri = "suppress_listing_violation";
		...
		/// </summary>
		/// <param name="requestBody">This type is the base request type of the SuppressViolation method.</param>
		public void SuppressViolation(SuppressViolationRequest requestBody)
		{
			var requestUri = "suppress_listing_violation";
			using var httpRequestMessage = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Post, requestUri);
			using var requestWriter = new System.IO.StringWriter();
