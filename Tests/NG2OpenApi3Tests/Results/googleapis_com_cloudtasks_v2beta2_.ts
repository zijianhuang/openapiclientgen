...
export namespace MyNS {
	...
	/** Request message for acknowledging a task using AcknowledgeTask. */
	export interface AcknowledgeTaskRequestFormProperties {

		/** Required. The task's current schedule time, available in the schedule_time returned by LeaseTasks response or RenewLease response. This restriction is to ensure that your worker currently holds the lease. */
		scheduleTime: FormControl<string | null | undefined>,
	}
	export function CreateAcknowledgeTaskRequestFormGroup() {
		return new FormGroup<AcknowledgeTaskRequestFormProperties>({
	...
	/** The status of a task attempt. */
	export interface AttemptStatusFormProperties {
	...
	export function CreateAttemptStatusFormGroup() {
		return new FormGroup<AttemptStatusFormProperties>({
	...
	/** The pull message contains data that can be used by the caller of LeaseTasks to process the task. This proto can only be used for tasks in a queue which has pull_target set. */
	export interface PullMessageFormProperties {
	...
	export function CreatePullMessageFormGroup() {
		return new FormGroup<PullMessageFormProperties>({
			payload: new FormControl<string | null | undefined>(undefined),
	...
	export interface TaskStatusFormProperties {
		...
		/** Output only. The number of attempts dispatched. This count includes attempts which have been dispatched but haven't received a response. */
		attemptDispatchCount: FormControl<number | null | undefined>,

		/** Output only. The number of attempts which have received a response. This field is not calculated for pull tasks. */
		attemptResponseCount: FormControl<number | null | undefined>,
	}
	export function CreateTaskStatusFormGroup() {
		return new FormGroup<TaskStatusFormProperties>({
			attemptDispatchCount: new FormControl<number | null | undefined>(undefined),
			attemptResponseCount: new FormControl<number | null | undefined>(undefined),
		});
	...
	/** Request message for canceling a lease using CancelLease. */
	export interface CancelLeaseRequestFormProperties {
	...
	export function CreateCancelLeaseRequestFormGroup() {
		return new FormGroup<CancelLeaseRequestFormProperties>({
	...
	/** Request message for leasing tasks using LeaseTasks. */
	export interface LeaseTasksRequestFormProperties {

		/** `filter` can be used to specify a subset of tasks to lease. When `filter` is set to `tag=` then the response will contain only tasks whose tag is equal to ``. `` must be less than 500 characters. When `filter` is set to `tag_function=oldest_tag()`, only tasks which have the same tag as the task with the oldest schedule_time will be returned. Grammar Syntax: * `filter = "tag=" tag | "tag_function=" function` * `tag = string` * `function = "oldest_tag()"` The `oldest_tag()` function returns tasks which have the same tag as the oldest task (ordered by schedule time). SDK compatibility: Although the SDK allows tags to be either string or [bytes](https://cloud.google.com/appengine/docs/standard/java/javadoc/com/google/appengine/api/taskqueue/TaskOptions.html#tag-byte:A-), only UTF-8 encoded tags can be used in Cloud Tasks. Tag which aren't UTF-8 encoded can't be used in the filter and the task's tag will be displayed as empty in Cloud Tasks. */
		filter: FormControl<string | null | undefined>,

		/** Required. The duration of the lease. Each task returned in the response will have its schedule_time set to the current time plus the `lease_duration`. The task is leased until its schedule_time; thus, the task will not be returned to another LeaseTasks call before its schedule_time. After the worker has successfully finished the work associated with the task, the worker must call via AcknowledgeTask before the schedule_time. Otherwise the task will be returned to a later LeaseTasks call so that another worker can retry it. The maximum lease duration is 1 week. `lease_duration` will be truncated to the nearest second. */
		leaseDuration: FormControl<string | null | undefined>,

		/** The maximum number of tasks to lease. The system will make a best effort to return as close to as `max_tasks` as possible. The largest that `max_tasks` can be is 1000. The maximum total size of a lease tasks response is 32 MB. If the sum of all task sizes requested reaches this limit, fewer tasks than requested are returned. */
		maxTasks: FormControl<number | null | undefined>,

		/** The response_view specifies which subset of the Task will be returned. By default response_view is BASIC; not all information is retrieved by default because some data, such as payloads, might be desirable to return only when needed because of its large size or because of the sensitivity of data that it contains. Authorization for FULL requires `cloudtasks.tasks.fullView` [Google IAM](https://cloud.google.com/iam/) permission on the Task resource. */
		responseView: FormControl<Task_View | null | undefined>,
	}
	export function CreateLeaseTasksRequestFormGroup() {
		return new FormGroup<LeaseTasksRequestFormProperties>({
			filter: new FormControl<string | null | undefined>(undefined),
			leaseDuration: new FormControl<string | null | undefined>(undefined),
			maxTasks: new FormControl<number | null | undefined>(undefined),
			responseView: new FormControl<Task_View | null | undefined>(undefined),
		});
	...
	/** Response message for leasing tasks using LeaseTasks. */
	export interface LeaseTasksResponseFormProperties {
	}
	export function CreateLeaseTasksResponseFormGroup() {
		return new FormGroup<LeaseTasksResponseFormProperties>({
		});
	...
	export interface QueueFormProperties {
		...
		/** The maximum amount of time that a task will be retained in this queue. Queues created by Cloud Tasks have a default `task_ttl` of 31 days. After a task has lived for `task_ttl`, the task will be deleted regardless of whether it was dispatched or not. The `task_ttl` for queues created via queue.yaml/xml is equal to the maximum duration because there is a [storage quota](https://cloud.google.com/appengine/quotas#Task_Queue) for these queues. To view the maximum valid duration, see the documentation for Duration. */
		taskTtl: FormControl<string | null | undefined>,

		/** The task tombstone time to live (TTL). After a task is deleted or completed, the task's tombstone is retained for the length of time specified by `tombstone_ttl`. The tombstone is used by task de-duplication; another task with the same name can't be created until the tombstone has expired. For more information about task de-duplication, see the documentation for CreateTaskRequest. Queues created by Cloud Tasks have a default `tombstone_ttl` of 1 hour. */
		tombstoneTtl: FormControl<string | null | undefined>,
	}
	export function CreateQueueFormGroup() {
		return new FormGroup<QueueFormProperties>({
			...
			taskTtl: new FormControl<string | null | undefined>(undefined),
			tombstoneTtl: new FormControl<string | null | undefined>(undefined),
		});
	...
	/** Pull target. */
	export interface PullTargetFormProperties {
	}
	export function CreatePullTargetFormGroup() {
		return new FormGroup<PullTargetFormProperties>({
		});
	...
	export interface RateLimitsFormProperties {
		...
		/** The maximum number of concurrent tasks that Cloud Tasks allows to be dispatched for this queue. After this threshold has been reached, Cloud Tasks stops dispatching tasks until the number of concurrent requests decreases. If unspecified when the queue is created, Cloud Tasks will pick the default. The maximum allowed value is 5,000. This field is output only for pull queues and always -1, which indicates no limit. No other queue types can have `max_concurrent_tasks` set to -1. This field has the same meaning as [max_concurrent_requests in queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#max_concurrent_requests). */
		maxConcurrentTasks: FormControl<number | null | undefined>,

		/** The maximum rate at which tasks are dispatched from this queue. If unspecified when the queue is created, Cloud Tasks will pick the default. * For App Engine queues, the maximum allowed value is 500. * This field is output only for pull queues. In addition to the `max_tasks_dispatched_per_second` limit, a maximum of 10 QPS of LeaseTasks requests are allowed per pull queue. This field has the same meaning as [rate in queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#rate). */
		maxTasksDispatchedPerSecond: FormControl<number | null | undefined>,
	}
	export function CreateRateLimitsFormGroup() {
		return new FormGroup<RateLimitsFormProperties>({
			...
			maxConcurrentTasks: new FormControl<number | null | undefined>(undefined),
			maxTasksDispatchedPerSecond: new FormControl<number | null | undefined>(undefined),
		});
	...
	export interface RetryConfigFormProperties {
		...
		/** If true, then the number of attempts is unlimited. */
		unlimitedAttempts: FormControl<boolean | null | undefined>,
	}
	export function CreateRetryConfigFormGroup() {
		return new FormGroup<RetryConfigFormProperties>({
			...
			unlimitedAttempts: new FormControl<boolean | null | undefined>(undefined),
		});
	...
	/** Statistics for a queue. */
	export interface QueueStatsFormProperties {

		/** Output only. The number of requests that the queue has dispatched but has not received a reply for yet. */
		concurrentDispatchesCount: FormControl<string | null | undefined>,

		/** Output only. The current maximum number of tasks per second executed by the queue. The maximum value of this variable is controlled by the RateLimits of the Queue. However, this value could be less to avoid overloading the endpoints tasks in the queue are targeting. */
		effectiveExecutionRate: FormControl<number | null | undefined>,

		/** Output only. The number of tasks that the queue has dispatched and received a reply for during the last minute. This variable counts both successful and non-successful executions. */
		executedLastMinuteCount: FormControl<string | null | undefined>,

		/** Output only. An estimation of the nearest time in the future where a task in the queue is scheduled to be executed. */
		oldestEstimatedArrivalTime: FormControl<string | null | undefined>,

		/** Output only. An estimation of the number of tasks in the queue, that is, the tasks in the queue that haven't been executed, the tasks in the queue which the queue has dispatched but has not yet received a reply for, and the failed tasks that the queue is retrying. */
		tasksCount: FormControl<string | null | undefined>,
	}
	export function CreateQueueStatsFormGroup() {
		return new FormGroup<QueueStatsFormProperties>({
			concurrentDispatchesCount: new FormControl<string | null | undefined>(undefined),
			effectiveExecutionRate: new FormControl<number | null | undefined>(undefined),
			executedLastMinuteCount: new FormControl<string | null | undefined>(undefined),
			oldestEstimatedArrivalTime: new FormControl<string | null | undefined>(undefined),
			tasksCount: new FormControl<string | null | undefined>(undefined),
		});
	...
	/** Request message for renewing a lease using RenewLease. */
	export interface RenewLeaseRequestFormProperties {
	...
	export function CreateRenewLeaseRequestFormGroup() {
		return new FormGroup<RenewLeaseRequestFormProperties>({
			leaseDuration: new FormControl<string | null | undefined>(undefined),
			responseView: new FormControl<Task_View | null | undefined>(undefined),
			scheduleTime: new FormControl<string | null | undefined>(undefined),
		});
	...
	export class MyClient {
		 ...
		 */
		Cloudtasks_api_queue_update(appId: string | null | undefined, requestBody: HttpBody): Observable<Empty> {
			return this.http.post<Empty>(this.baseUri + 'api/queue/update?appId=' + (appId == null ? '' : encodeURIComponent(appId)), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_tasks_delete(name: string): Observable<Empty> {
			return this.http.delete<Empty>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)), {});
		}
		...
		Cloudtasks_projects_locations_queues_tasks_get(name: string, responseView: Task_View | null | undefined): Observable<Task_> {
			return this.http.get<Task_>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + '&responseView=' + responseView, {});
		}
		...
		Cloudtasks_projects_locations_queues_patch(name: string, updateMask: string | null | undefined, requestBody: Queue): Observable<Queue> {
			return this.http.patch<Queue>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + '&updateMask=' + (updateMask == null ? '' : encodeURIComponent(updateMask)), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_list(name: string, filter: string | null | undefined, pageSize: number | null | undefined, pageToken: string | null | undefined): Observable<ListLocationsResponse> {
			return this.http.get<ListLocationsResponse>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + '/locations&filter=' + (filter == null ? '' : encodeURIComponent(filter)) + '&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)), {});
		}
		 ...
		 * @return {Empty} Successful response
		 */
		Cloudtasks_projects_locations_queues_tasks_acknowledge(name: string, requestBody: AcknowledgeTaskRequest): Observable<Empty> {
			return this.http.post<Empty>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':acknowledge', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 * @return {Task_} Successful response
		 */
		Cloudtasks_projects_locations_queues_tasks_cancelLease(name: string, requestBody: CancelLeaseRequest): Observable<Task_> {
			return this.http.post<Task_>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':cancelLease', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_pause(name: string, requestBody: PauseQueueRequest): Observable<Queue> {
			return this.http.post<Queue>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':pause', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_purge(name: string, requestBody: PurgeQueueRequest): Observable<Queue> {
			return this.http.post<Queue>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':purge', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 * @return {Task_} Successful response
		 */
		Cloudtasks_projects_locations_queues_tasks_renewLease(name: string, requestBody: RenewLeaseRequest): Observable<Task_> {
			return this.http.post<Task_>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':renewLease', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_resume(name: string, requestBody: ResumeQueueRequest): Observable<Queue> {
			return this.http.post<Queue>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':resume', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_tasks_run(name: string, requestBody: RunTaskRequest): Observable<Task_> {
			return this.http.post<Task_>(this.baseUri + 'v2beta2/' + (name == null ? '' : encodeURIComponent(name)) + ':run', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 */
		Cloudtasks_projects_locations_queues_list(parent: string, filter: string | null | undefined, pageSize: number | null | undefined, pageToken: string | null | undefined, readMask: string | null | undefined): Observable<ListQueuesResponse> {
			return this.http.get<ListQueuesResponse>(this.baseUri + 'v2beta2/' + (parent == null ? '' : encodeURIComponent(parent)) + '/queues&filter=' + (filter == null ? '' : encodeURIComponent(filter)) + '&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)) + '&readMask=' + (readMask == null ? '' : encodeURIComponent(readMask)), {});
		}
		...
		Cloudtasks_projects_locations_queues_create(parent: string, requestBody: Queue): Observable<Queue> {
			return this.http.post<Queue>(this.baseUri + 'v2beta2/' + (parent == null ? '' : encodeURIComponent(parent)) + '/queues', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_tasks_list(parent: string, pageSize: number | null | undefined, pageToken: string | null | undefined, responseView: Task_View | null | undefined): Observable<ListTasksResponse> {
			return this.http.get<ListTasksResponse>(this.baseUri + 'v2beta2/' + (parent == null ? '' : encodeURIComponent(parent)) + '/tasks&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)) + '&responseView=' + responseView, {});
		}
		...
		Cloudtasks_projects_locations_queues_tasks_create(parent: string, requestBody: CreateTaskRequest): Observable<Task_> {
			return this.http.post<Task_>(this.baseUri + 'v2beta2/' + (parent == null ? '' : encodeURIComponent(parent)) + '/tasks', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 * Post v2beta2/{parent}/tasks:lease
		 * @param {string} parent Required. The queue name. For example: `projects/PROJECT_ID/locations/LOCATION_ID/queues/QUEUE_ID`
		 * @return {LeaseTasksResponse} Successful response
		 */
		Cloudtasks_projects_locations_queues_tasks_lease(parent: string, requestBody: LeaseTasksRequest): Observable<LeaseTasksResponse> {
			return this.http.post<LeaseTasksResponse>(this.baseUri + 'v2beta2/' + (parent == null ? '' : encodeURIComponent(parent)) + '/tasks:lease', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}

		/**
		 * Creates and buffers a new task without the need to explicitly define a Task message. The queue must have HTTP target. To create the task with a custom ID, use the following format and set TASK_ID to your desired ID: projects/PROJECT_ID/locations/LOCATION_ID/queues/QUEUE_ID/tasks/TASK_ID:buffer To create the task with an automatically generated ID, use the following format: projects/PROJECT_ID/locations/LOCATION_ID/queues/QUEUE_ID/tasks:buffer.
		 * Post v2beta2/{queue}/tasks/{taskId}:buffer
		...
		Cloudtasks_projects_locations_queues_tasks_buffer(queue: string, taskId: string, requestBody: BufferTaskRequest): Observable<BufferTaskResponse> {
			return this.http.post<BufferTaskResponse>(this.baseUri + 'v2beta2/' + (queue == null ? '' : encodeURIComponent(queue)) + '/tasks/' + (taskId == null ? '' : encodeURIComponent(taskId)) + ':buffer', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_getIamPolicy(resource: string, requestBody: GetIamPolicyRequest): Observable<Policy> {
			return this.http.post<Policy>(this.baseUri + 'v2beta2/' + (resource == null ? '' : encodeURIComponent(resource)) + ':getIamPolicy', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_setIamPolicy(resource: string, requestBody: SetIamPolicyRequest): Observable<Policy> {
			return this.http.post<Policy>(this.baseUri + 'v2beta2/' + (resource == null ? '' : encodeURIComponent(resource)) + ':setIamPolicy', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		...
		Cloudtasks_projects_locations_queues_testIamPermissions(resource: string, requestBody: TestIamPermissionsRequest): Observable<TestIamPermissionsResponse> {
			return this.http.post<TestIamPermissionsResponse>(this.baseUri + 'v2beta2/' + (resource == null ? '' : encodeURIComponent(resource)) + ':testIamPermissions', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
	}
