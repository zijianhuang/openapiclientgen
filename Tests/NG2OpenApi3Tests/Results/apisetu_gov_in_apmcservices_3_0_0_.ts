...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {ApmclPostBody} requestBody Request format
		 * @return {void} The certificate data in response body in PDF, XML or JSON format as requested in format parameter.
		 */
		Apmcl(requestBody: ApmclPostBody): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'apmcl/certificate', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
	}

	export interface ApmclPostBody {
		certificateParameters?: ApmclPostBodyCertificateParameters;
	...
	export interface ApmclPostBodyFormProperties {

		/**
		 * The format of the certificate in response.
		 * Required
		 */
		format: FormControl<ApmclPostBodyFormat | null | undefined>,
	...
	export function CreateApmclPostBodyFormGroup() {
		return new FormGroup<ApmclPostBodyFormProperties>({
			format: new FormControl<ApmclPostBodyFormat | null | undefined>(undefined, [Validators.required]),
			txnId: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export interface ApmclPostBodyCertificateParameters {
	...
	export interface ApmclPostBodyCertificateParametersFormProperties {
	...
	export function CreateApmclPostBodyCertificateParametersFormGroup() {
		return new FormGroup<ApmclPostBodyCertificateParametersFormProperties>({
			DocumentNumber: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export enum ApmclPostBodyFormat { pdf = 'pdf' }
