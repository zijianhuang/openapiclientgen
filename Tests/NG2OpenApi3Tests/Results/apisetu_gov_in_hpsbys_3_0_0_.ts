...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {RsbycPostBody} requestBody Request format
		 * @return {void} The certificate data in response body in PDF, XML or JSON format as requested in format parameter.
		 */
		Rsbyc(requestBody: RsbycPostBody): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'rsbyc/certificate', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
	}

	export interface RsbycPostBody {
		certificateParameters?: RsbycPostBodyCertificateParameters;
	...
	export interface RsbycPostBodyFormProperties {

		/**
		 * The format of the certificate in response.
		 * Required
		 */
		format: FormControl<RsbycPostBodyFormat | null | undefined>,
	...
	export function CreateRsbycPostBodyFormGroup() {
		return new FormGroup<RsbycPostBodyFormProperties>({
			format: new FormControl<RsbycPostBodyFormat | null | undefined>(undefined, [Validators.required]),
			txnId: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export interface RsbycPostBodyCertificateParameters {
	...
	export interface RsbycPostBodyCertificateParametersFormProperties {
	...
	export function CreateRsbycPostBodyCertificateParametersFormGroup() {
		return new FormGroup<RsbycPostBodyCertificateParametersFormProperties>({
			UID: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});
