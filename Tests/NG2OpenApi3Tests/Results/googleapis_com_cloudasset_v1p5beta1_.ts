...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {Cloudasset_assets_listContentType} contentType Asset content type. If not specified, no content but the asset name will be returned.
		 ...
		 */
		Cloudasset_assets_list(parent: string, assetTypes: Array<string> | null | undefined, contentType: Cloudasset_assets_listContentType | null | undefined, pageSize: number | null | undefined, pageToken: string | null | undefined, readTime: string | null | undefined): Observable<ListAssetsResponse> {
			return this.http.get<ListAssetsResponse>(this.baseUri + 'v1p5beta1/' + (parent == null ? '' : encodeURIComponent(parent)) + '/assets&' + assetTypes?.map(z => `assetTypes=${encodeURIComponent(z)}`).join('&') + '&contentType=' + contentType + '&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)) + '&readTime=' + (readTime == null ? '' : encodeURIComponent(readTime)), {});
		}
	}

	export enum Cloudasset_assets_listContentType { CONTENT_TYPE_UNSPECIFIED = 'CONTENT_TYPE_UNSPECIFIED', RESOURCE = 'RESOURCE', IAM_POLICY = 'IAM_POLICY', ORG_POLICY = 'ORG_POLICY', ACCESS_POLICY = 'ACCESS_POLICY' }
