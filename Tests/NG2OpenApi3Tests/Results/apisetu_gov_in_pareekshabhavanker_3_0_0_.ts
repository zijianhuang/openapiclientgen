...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {SslcrPostBody} requestBody Request format
		 * @return {void} The certificate data in response body in PDF, XML or JSON format as requested in format parameter.
		 */
		Sslcr(requestBody: SslcrPostBody): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'sslcr/certificate', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
	}

	export interface SslcrPostBody {
		certificateParameters?: SslcrPostBodyCertificateParameters;
	...
	export interface SslcrPostBodyFormProperties {

		/**
		 * The format of the certificate in response.
		 * Required
		 */
		format: FormControl<SslcrPostBodyFormat | null | undefined>,
	...
	export function CreateSslcrPostBodyFormGroup() {
		return new FormGroup<SslcrPostBodyFormProperties>({
			format: new FormControl<SslcrPostBodyFormat | null | undefined>(undefined, [Validators.required]),
			txnId: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export interface SslcrPostBodyCertificateParameters {
	...
	export interface SslcrPostBodyCertificateParametersFormProperties {
	...
	export function CreateSslcrPostBodyCertificateParametersFormGroup() {
		return new FormGroup<SslcrPostBodyCertificateParametersFormProperties>({
	...
	export enum SslcrPostBodyFormat { pdf = 'pdf' }
