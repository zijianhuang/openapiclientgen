...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {NncerPostBody} requestBody Request format
		 * @return {void} The certificate data in response body in PDF, XML or JSON format as requested in format parameter.
		 */
		Nncer(requestBody: NncerPostBody): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'nncer/certificate', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
	}

	export interface NncerPostBody {
		certificateParameters?: NncerPostBodyCertificateParameters;
	...
	export interface NncerPostBodyFormProperties {

		/**
		 * The format of the certificate in response.
		 * Required
		 */
		format: FormControl<NncerPostBodyFormat | null | undefined>,
	...
	export function CreateNncerPostBodyFormGroup() {
		return new FormGroup<NncerPostBodyFormProperties>({
			format: new FormControl<NncerPostBodyFormat | null | undefined>(undefined, [Validators.required]),
			txnId: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export interface NncerPostBodyCertificateParameters {
	...
	export interface NncerPostBodyCertificateParametersFormProperties {
	...
	export function CreateNncerPostBodyCertificateParametersFormGroup() {
		return new FormGroup<NncerPostBodyCertificateParametersFormProperties>({
			RefNo: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export enum NncerPostBodyFormat { pdf = 'pdf' }
