...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @param {DipcrPostBody} requestBody Request format
		 * @return {void} The certificate data in response body in PDF, XML or JSON format as requested in format parameter.
		 */
		Dipcr(requestBody: DipcrPostBody): Observable<HttpResponse<string>> {
			return this.http.post(this.baseUri + 'dipcr/certificate', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' }, observe: 'response', responseType: 'text' });
		}
	}

	export interface DipcrPostBody {
		certificateParameters?: DipcrPostBodyCertificateParameters;
	...
	export interface DipcrPostBodyFormProperties {

		/**
		 * The format of the certificate in response.
		 * Required
		 */
		format: FormControl<DipcrPostBodyFormat | null | undefined>,
	...
	export function CreateDipcrPostBodyFormGroup() {
		return new FormGroup<DipcrPostBodyFormProperties>({
			format: new FormControl<DipcrPostBodyFormat | null | undefined>(undefined, [Validators.required]),
			txnId: new FormControl<string | null | undefined>(undefined, [Validators.required]),
		});

	}

	export interface DipcrPostBodyCertificateParameters {
	...
	export interface DipcrPostBodyCertificateParametersFormProperties {
	...
	export function CreateDipcrPostBodyCertificateParametersFormGroup() {
		return new FormGroup<DipcrPostBodyCertificateParametersFormProperties>({
	...
	export enum DipcrPostBodyFormat { pdf = 'pdf' }
