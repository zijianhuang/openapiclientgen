...
export namespace MyNS {
	...
	/** AnalyzePackagesRequest is the request to analyze a list of packages and create Vulnerability Occurrences for it. */
	export interface AnalyzePackagesRequestFormProperties {

		/** Required. The resource URI of the container image being scanned. */
		resourceUri: FormControl<string | null | undefined>,
	}
	export function CreateAnalyzePackagesRequestFormGroup() {
		return new FormGroup<AnalyzePackagesRequestFormProperties>({
			resourceUri: new FormControl<string | null | undefined>(undefined),
		});
	...
	/** ListVulnerabilitiesResponse contains a single page of vulnerabilities resulting from a scan. */
	export interface ListVulnerabilitiesResponseFormProperties {

		/** A page token that can be used in a subsequent call to ListVulnerabilities to continue retrieving results. */
		nextPageToken: FormControl<string | null | undefined>,
	}
	export function CreateListVulnerabilitiesResponseFormGroup() {
		return new FormGroup<ListVulnerabilitiesResponseFormProperties>({
			nextPageToken: new FormControl<string | null | undefined>(undefined),
		});
	...
	export class MyClient {
		...
		Ondemandscanning_projects_locations_operations_wait(name: string, timeout: string | null | undefined): Observable<Operation> {
			return this.http.post<Operation>(this.baseUri + 'v1beta1/' + (name == null ? '' : encodeURIComponent(name)) + ':wait&timeout=' + (timeout == null ? '' : encodeURIComponent(timeout)), null, {});
		}
		 ...
		 * @return {Operation} Successful response
		 */
		Ondemandscanning_projects_locations_scans_analyzePackages(parent: string, requestBody: AnalyzePackagesRequest): Observable<Operation> {
			return this.http.post<Operation>(this.baseUri + 'v1beta1/' + (parent == null ? '' : encodeURIComponent(parent)) + '/scans:analyzePackages', JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
		 ...
		 */
		Ondemandscanning_projects_locations_scans_vulnerabilities_list(parent: string, pageSize: number | null | undefined, pageToken: string | null | undefined): Observable<ListVulnerabilitiesResponse> {
			return this.http.get<ListVulnerabilitiesResponse>(this.baseUri + 'v1beta1/' + (parent == null ? '' : encodeURIComponent(parent)) + '/vulnerabilities&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)), {});
		}
	}
