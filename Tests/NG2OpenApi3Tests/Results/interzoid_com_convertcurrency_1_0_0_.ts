...
export namespace MyNS {
	...
	export class MyClient {
		 ...
		 * @return {ConvertcurrencyReturn} Currency rate data to one US DOllar
		 */
		Convertcurrency(license: string, from: string, to: string, amount: string): Observable<ConvertcurrencyReturn> {
			return this.http.get<ConvertcurrencyReturn>(this.baseUri + 'convertcurrency?license=' + (license == null ? '' : encodeURIComponent(license)) + '&from=' + (from == null ? '' : encodeURIComponent(from)) + '&to=' + (to == null ? '' : encodeURIComponent(to)) + '&amount=' + (amount == null ? '' : encodeURIComponent(amount)), {});
		}
	}

	export interface ConvertcurrencyReturn {
		Code?: string | null;
		Converted?: string | null;
		Credits?: string | null;
		Currency?: string | null;
	}
	export interface ConvertcurrencyReturnFormProperties {
		Code: FormControl<string | null | undefined>,
		Converted: FormControl<string | null | undefined>,
		Credits: FormControl<string | null | undefined>,
		Currency: FormControl<string | null | undefined>,
	}
	export function CreateConvertcurrencyReturnFormGroup() {
		return new FormGroup<ConvertcurrencyReturnFormProperties>({
