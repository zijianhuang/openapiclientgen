...
export namespace MyNS {
	...
	export class MyClient {
		...
		Vpcaccess_projects_locations_connectors_list(parent: string, pageSize: number | null | undefined, pageToken: string | null | undefined): Observable<ListConnectorsResponse> {
			return this.http.get<ListConnectorsResponse>(this.baseUri + 'v1/' + (parent == null ? '' : encodeURIComponent(parent)) + '/connectors&pageSize=' + pageSize + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)), {});
		}
		...
		Vpcaccess_projects_locations_connectors_create(parent: string, connectorId: string | null | undefined, requestBody: Connector): Observable<Operation> {
			return this.http.post<Operation>(this.baseUri + 'v1/' + (parent == null ? '' : encodeURIComponent(parent)) + '/connectors&connectorId=' + (connectorId == null ? '' : encodeURIComponent(connectorId)), JSON.stringify(requestBody), { headers: { 'Content-Type': 'application/json;charset=UTF-8' } });
		}
	}
